...
##定时任务：-------------
##每5分钟跑一次脚本，日志输出到monitor.log文件
#crontab -e
##输入以下内容：
#*/5 * * * * sh /home/admin/monitor-jstat.sh > /data/logs/monitor/monitor.log
...
## 可以指定忽略某些进程比如Jps和logstash-watch-collection应用，使用grep -v 参数
for pid in `jps -v |grep -v Jps |grep -v logstash-watch-collection |awk '{print $1}'`

do
 ## 应用名称 忽略Jps和 logstash-watch-collection应用
 ...
 then
   ...
   #echo "JVM告警信息：${jvm}"
   smsContent='{"mobile":"'$mobile'","content":"'$jvm'","userid":"11111","pwd":"123456"}'
